...
PROVIDES = "\
	...
	virtual/libiconv \
	glibc-thread-db \
	${@base_conditional('PREFERRED_PROVIDER_linux-libc-headers', 'external-toolchain-trident', 'linux-libc-headers', '', d)} \
	${@base_conditional('PREFERRED_PROVIDER_gdbserver', 'external-toolchain-trident', 'gdbserver', '', d)} \
"

DEPENDS = "${@base_conditional('PREFERRED_PROVIDER_linux-libc-headers', 'external-toolchain-trident', '', 'linux-libc-headers', d)}"
...
def ang_get_libc_version(d):
	import os,bb
	if os.path.exists(bb.data.getVar('TOOLCHAIN_SYSPATH', d, 1)+'/sys-root/lib/'):
		for file in os.listdir(bb.data.getVar('TOOLCHAIN_SYSPATH', d, 1)+'/sys-root/lib/'):
			if file.find('libc-') == 0:
				return file[5:-3]
		return None
...
def ang_get_kernel_version(d):
	import os,bb
	if os.path.exists(bb.data.getVar('TOOLCHAIN_SYSPATH', d, 1)+'/sys-root/usr/include/linux/'):
		f = open(bb.data.getVar('TOOLCHAIN_SYSPATH', d, 1)+'/sys-root/usr/include/linux/version.h', 'r')
		l = f.readlines();
		f.close();
		for s in l:
...
do_install() {
	...
	cp -a ${TOOLCHAIN_PATH}/${TARGET_SYS}/sys-root/${base_libdir}/{lib*,ld*} ${D}${base_libdir}
	cp -a ${TOOLCHAIN_PATH}/${TARGET_SYS}/sys-root/${base_sbindir}/ldconfig ${D}${base_sbindir}
	cp -a ${TOOLCHAIN_PATH}/${TARGET_SYS}/sys-root/${bindir}/{gencat,getconf,getent,iconv,locale,mtrace,pcprofiledump,rpcgen,sprof,tzselect,xtrace} ${D}${bindir}
	cp -a ${TOOLCHAIN_PATH}/${TARGET_SYS}/sys-root/${sbindir}/{iconvconfig,rpcinfo,zdump,zic} ${D}${sbindir}
	cp -a ${TOOLCHAIN_PATH}/${TARGET_SYS}/sys-root/${includedir}/{arpa,asm*,bits,drm,gnu,linux,mtd,net*,nfs,protocols,rdma,rpc*,scsi,sound,sys*,video,*.h} ${D}${includedir}
	cp -a ${TOOLCHAIN_PATH}/${TARGET_SYS}/sys-root/${libdir}/{?crt1.o,crt?.o,libBrokenLocale*,libanl*,libc.*,libc_*,libcrypt.*,,libdl.*,libg.*,libieee.*,libm.*,libmcheck.*,libnsl*,libnss*,libpthread*,libresolv*,librt*,libthread*,libutil*} ${D}${libdir}
	cp -a ${TOOLCHAIN_PATH}/${TARGET_SYS}/sys-root/${base_libdir}/libstdc* ${D}${libdir}
	rm -rf ${D}${base_libdir}/*.la
	rm -rf ${D}${libdir}/*.la

	${@base_conditional('PREFERRED_PROVIDER_linux-libc-headers', 'external-toolchain-trident', '', 'rm -rf ${D}/usr/include/linux', d)}
